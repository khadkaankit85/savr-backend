...
import { scrapeBestBuy, scrapeGiantTiger, scrapeCadTire } from "../scrapes/scraper";
...
router.get("/scrape", async (req: Request, res: Response) => {
    ...
    try {
        ...
        const cadTireData = results[2].status === 'fulfilled' ? results[2].value : { error: 'Failed to fetch GiantTiger data', details: results[2].reason };
